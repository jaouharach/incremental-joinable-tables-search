        ...
        for gt_file in files:
            if re.search(f'_runtime', gt_file):
                ...
                if gt_query == query:
                    ...
                    recall = compute_recall_on_distance(query_results_csv_file, ground_truth_dir+gt_file)
...
def compute_recall_on_distance(csv_file, gt_csv_file):
    ...
    # sort by query vector position and by distance
    gt_results = gt_results.sort_values('q_pos')
    results = results.sort_values('q_pos')

    gt_results = gt_results.sort_values('d')
    results = results.sort_values('d')

    match_count = 0

    for r in range(total_rows):
        if math.isclose(float(gt_results.loc[r, "d"]), float(results.loc[r, "d"]), rel_tol=1e-5):
            match_count += 1
...
def plot_results(csv_file, querytime_csv_file, output_dir, k_count):
    ...
    plt.savefig(f"{output_dir}/kashif_recall_querytime_on_dist.png")
    plt.close()
...
csv_file = "./csv/recall_eval_100k_new.csv"
