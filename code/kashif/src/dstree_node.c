          ...
          float warping)
{
     ...
     unsigned int ts_byte_size = sizeof(ts_type) * index->settings->timeseries_size;
    ...
    for (unsigned int idx = 0; idx < node->file_buffer->buffered_list_size; ++idx)
    {  
      ...
      if(warping > 0){
             distance = ts_warping_distance(query_ts_reordered,
                    ...
                    query_order,
                    warping);
            // distance = get_dtw(query_ts_reordered, query_order, node->file_buffer->buffered_list[idx], warping*index->settings->timeseries_size, index->settings->timeseries_size, kth_bsf);
      }
      else{
    ...
    //if interested in the value of the actual neighbor, move this code inside the for loop above
	...
	for (int j = 0; j < k; ++j)
	  {
	    ...
	    if (index->settings->track_file_pos){
	      bsf_snapshots[j][*cur_bsf_snapshot].file_pos = knn_results[j].file_pos;	    
 	      //bsf_snapshots[j][*cur_bsf_snapshot].series = calloc (1, ts_byte_size);
...
ts_type calculate_node_min_distance_warping (struct dstree_index *index, struct dstree_node *node, ts_type *query, float warping)
{
    // Window fraction = 5%
    ...
    for(int i = 0; i < num_points; ++i){
        ...
        if(node->node_segment_sketches[i].indicators[1] > u_hat[i]){
          temp_dist += pow(node->node_segment_sketches[i].indicators[1] - u_hat[i], 2);
        }
        // mu_max < l_hat
        if(node->node_segment_sketches[i].indicators[0] < l_hat[i]){
          temp_dist += pow(l_hat[i] - node->node_segment_sketches[i].indicators[0], 2);
        }
