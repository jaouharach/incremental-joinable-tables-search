...
enum response
mean_node_segment_split_policy_split(struct node_segment_split_policy *policy,
                                     struct segment_sketch sketch,
  ...
  policy->indicator_split_value =
      (max_mean + min_mean) / 2; // the mean value is split value
...
enum response
stdev_node_segment_split_policy_split(struct node_segment_split_policy *policy,
                                      struct segment_sketch sketch,
                                      struct segment_sketch *ret) {
  ...
  policy->indicator_split_value = (ts_type)(max_stdev + min_stdev) /
                                  2; // the mean of stdev value is split value
...
enum response
series_segment_sketch_do_sketch(struct segment_sketch *series_segment_sketch,
                                ts_type *series, int fromIdx, int toIdx) {
...
enum response
node_segment_sketch_update_sketch(struct segment_sketch *node_segment_sketch,
                                  ts_type *series, int fromIdx, int toIdx) {
  ...
  if (series_segment_sketch.indicators == NULL) {
    fprintf(
        stderr,
        "Error in dstree_node_split.c: Could not allocate memory for series \
                        segment sketch indicators.\n");
    return FAILURE;
  }
  series_segment_sketch.num_indicators = 2;

  if (!series_segment_sketch_do_sketch(&series_segment_sketch, series, fromIdx,
                                       toIdx)) {
    fprintf(
        stderr,
        "Error in node_init_segments(): Could not calculate the series segment  \
                    sketch.\n");
    return FAILURE;
  }
  ...
  if (node_segment_sketch->indicators == NULL) {
    ...
    if (node_segment_sketch->indicators == NULL) {
      fprintf(
          stderr,
          "Error in dstree_node_split.c: Could not allocate memory for node \
                        segment sketch indicators.\n");
      return FAILURE;
    }
  ...
  node_segment_sketch->indicators[0] = fmaxf(
      node_segment_sketch->indicators[0], series_segment_sketch.indicators[0]);
  node_segment_sketch->indicators[1] = fminf(
      node_segment_sketch->indicators[1], series_segment_sketch.indicators[0]);
  node_segment_sketch->indicators[2] = fmaxf(
      node_segment_sketch->indicators[2], series_segment_sketch.indicators[1]);
  node_segment_sketch->indicators[3] = fminf(
      node_segment_sketch->indicators[3], series_segment_sketch.indicators[1]);
  node_segment_sketch->num_indicators = 4;
...
boolean node_split_policy_route_to_left(struct dstree_node *node,
                                        ts_type *series) {
  ...
  if (!series_segment_sketch_do_sketch(&series_segment_sketch, series,
    ...
    fprintf(stderr,
            "Error in dstree_node_split.c: Could not calculate the series  \
                        segment sketch .\n");
  }

  if (series_segment_sketch.indicators[policy.indicator_split_idx] <
      policy.indicator_split_value) {
    route_to_left = true;
  }
...
enum response split_node(struct dstree_index *index, struct dstree_node *node,
                         short *child_node_points, int num_child_node_points) {
  if (!node->is_leaf) {
    ...
    node->left_child = create_child_node(node, index);
    ...
    if (!node_init_segments(node->left_child, child_node_points,
                            num_child_node_points)) {
      fprintf(stderr,
              "Error in dstree_node_split.c: Could not initialize       \
    ...
    node->right_child = create_child_node(node, index);
    ...
    if (!create_dstree_node_filename(index->settings, node->right_child,
                                     node)) {
      fprintf(stderr, "Error in dstree_node_split.c: Could not create        \
...
struct dstree_node *create_child_node(struct dstree_node *parent,
  ...
  struct dstree_node *child_node = dstree_leaf_node_init(index->settings);
  ...
  child_node->num_node_segment_split_policies =
      parent->num_node_segment_split_policies;
...
short get_hs_split_point(short *points, short from, short to, int size_points) {
  ...
  res =
      (short *)bsearch(&to, points, size_points, sizeof(short), compare_short);
...
enum response calc_split_points(short *points, int ts_length,
                                int segment_size) {
...
enum response calc_hs_split_points(short *hs_split_points,
                                   int *num_hs_split_points,
                                   short *split_points, int segment_size,
                                   int min_length) {
  int c = 0;
