...
void approximate_knn_search (ts_type *query_ts, ts_type * query_ts_reordered,
    ...
    if (node != NULL)
    {
	  ...
	  calculate_node_knn_distance(index, node, query_ts_reordered, query_order,offset,
				      bsf,k,knn_results,bsf_snapshots, cur_bsf_snapshot,curr_size,warping);
    }
    else
    {
        printf("Error: index is empty \n");        
...
struct query_result exact_search (ts_type *query_ts, ts_type * query_ts_reordered, int * query_order, unsigned int offset, struct dstree_index *index,ts_type minimum_distance, ts_type epsilon, ts_type delta) {
    ...
    root_pq_item->distance = calculate_node_min_distance (index, index->first_node, query_ts,0);
      ...
      if (n->distance > bsf_result.distance/(1 + epsilon))
	{
	  ...
	  child_distance = calculate_node_min_distance(index, n->node->left_child,query_ts,0);
	  ...
	  child_distance = calculate_node_min_distance(index, n->node->right_child,query_ts,0);
      ...
      if (n->distance > kth_bsf/(1 + epsilon))
	{
	  ...
	  //upon return, the queue will update the next best (k-foundkNN)th objects
 	  calculate_node_knn_distance(index, n->node, query_ts_reordered,
				      ...
				      k,knn_results,NULL, NULL,&curr_size,0);
				  ...
				  struct bsf_snapshot ** bsf_snapshots, unsigned int * cur_bsf_snapshot)
{
    ...
    //return k approximate results
    approximate_knn_search(query_ts, query_ts_reordered, query_order, offset, bsf, index,knn_results,k,
			   bsf_snapshots, cur_bsf_snapshot,&curr_size,0);
      ...
      if (found_knn == k)
	{
	  ...
	  //upon return, the queue will update the next best (k-foundkNN)th objects
 	  calculate_node_knn_distance(index, n->node, query_ts_reordered,
				      query_order, offset, bsf_result.distance,
				      k,knn_results,bsf_snapshots,cur_bsf_snapshot,&curr_size,0);
        }
      // If it is an intermediate node calculate mindist for children
      // and push them in the queue
      else  //n is an internal node
...
void  exact_de_progressive_knn_search (ts_type *query_ts, ts_type * query_ts_reordered,
				       int * query_order, unsigned int offset,
				       struct dstree_index *index,ts_type minimum_distance,
				       ts_type epsilon, ts_type r_delta,
				       unsigned int k, unsigned int q_id, char * qfilename,
				       struct bsf_snapshot ** bsf_snapshots, unsigned int * cur_bsf_snapshot)
{
  ...
  // Free the nodes that were not popped.
	...
	get_query_stats(index, found_knn);
	print_query_stats(index, q_id, found_knn,qfilename);	
	print_perk_progressive_bsf_snapshots(index, q_id,found_knn,qfilename,bsf_snapshots, *cur_bsf_snapshot, bsf_result.distance, NULL, NULL, NULL);
	//report all results for found_knn - last_found_knn or print their results
	RESET_QUERY_COUNTERS()
        RESET_PARTIAL_COUNTERS()
	//COUNT_PARTIAL_TIME_START	
	...
	bsf_snapshots[idx][*cur_bsf_snapshot].time = partial_time;
      ...
      ++(*cur_bsf_snapshot);
    */
    //print_progressive_bsf_snapshots(index, q_id,k,qfilename,bsf_snapshots, *cur_bsf_snapshot);
...
void  exact_de_incr_progressive_knn_search (ts_type *query_ts, ts_type * query_ts_reordered,
					    int * query_order, unsigned int offset,
					    struct dstree_index *index,ts_type minimum_distance,
					    ts_type epsilon, ts_type r_delta,
					    unsigned int k, unsigned int q_id, char * qfilename,
					    struct bsf_snapshot ** bsf_snapshots, unsigned int * cur_bsf_snapshot, float warping, FILE * dataset_file,FILE * series_file )
{
    ...
    ts_type *series = calloc(1,sizeof(ts_type)*index->settings->timeseries_size);
    //queue containing kNN results
    struct query_result * knn_results = calloc(k,sizeof(struct query_result));
    for (int idx = 0; idx < k; ++idx)
    {
    ...
    approximate_knn_search(query_ts, query_ts_reordered, query_order, offset, bsf,
			   index,knn_results,k,
			   bsf_snapshots, cur_bsf_snapshot,&curr_size,warping);
    ...
    struct query_result * root_pq_item = malloc(sizeof(struct query_result));
    root_pq_item->node = index->first_node;
    root_pq_item->distance = calculate_node_min_distance (index, index->first_node, query_ts, warping);
	...
	if (n->distance > bsf_result.distance) //add epsilon+1
	{
	  ...
	  update_query_stats(index,q_id, found_knn, bsf_result);
          get_query_stats(index, found_knn);
          print_query_stats(index, q_id, found_knn,qfilename);
	  print_perk_progressive_bsf_snapshots(index, q_id,found_knn,qfilename,bsf_snapshots, *cur_bsf_snapshot, bsf_result.distance, dataset_file, series_file, series);
	  //print_perk_progressive_bsf_snapshots(index, q_id,found_knn,qfilename,bsf_snapshots, *cur_bsf_snapshot, bsf_result.distance, NULL, NULL);
	  //printf("found NN = %u\n", found_knn);
	  //fflush(stdout);
	  //reset the bsf for the next NN
	  if (found_knn < k){	    
      ...
      if (found_knn >= k)
	{
      ...
      // and push them in the queue
      else  //n is an internal node
        {
          ...
          ts_type child_distance;
	  child_distance = calculate_node_min_distance(index, n->node->left_child,query_ts, warping);
	...
	if ((child_distance < kth_bsf) &&
	  ...
	  child_distance = calculate_node_min_distance(index, n->node->right_child,query_ts, warping);
...
void print_progressive_bsf_snapshots(struct dstree_index * index, unsigned int query_num, unsigned int k,
	  ...
	  for (unsigned int j = 0; j < cur_bsf_snapshot; ++j)
	      ...
	      if (j > 0)
		{
	      ...
	      printf("Query_bsf_snapshot_checked_nodes_count\t%lu\t%s\t%u\t%u\n",
		     bsf_snapshots[i][j].checked_nodes,
		     queries,
		     query_num,
		     i+1	       
		     );		      
...
void print_perk_progressive_bsf_snapshots(struct dstree_index * index, unsigned int query_num, unsigned int found_knn,
					  char * queries, struct bsf_snapshot **bsf_snapshots, unsigned int cur_bsf_snapshot, ts_type exact_distance, FILE  * dataset_file, FILE * series_file, ts_type *series)
{
  ...
  for (unsigned int j = 0; j < cur_bsf_snapshot; ++j)
    {
      ...
      if (j > 0)
      {
	if ((bsf_snapshots [found_knn-1][j].distance == exact_distance) && (bsf_snapshots[found_knn-1][j-1].distance == exact_distance))
	  {	  
	    break;
	  }
      }
      */
      if (j > 0)
      {
	if ((bsf_snapshots [found_knn-1][j].distance == bsf_snapshots[found_knn-1][j-1].distance))
	  {	  
	    continue;
	  }
      }      
      printf("Query_bsf_snapshot_time_secs\t%lf\t%s\t%u\t%u\n",
	     bsf_snapshots[found_knn-1][j].time/1000000,	        
	     queries,
	     query_num,
	     found_knn
	     );
      ...
      printf("Query_bsf_snapshot_distance\t%lf\t%s\t%u\t%u\n",
	     sqrtf(bsf_snapshots[found_knn-1][j].distance), 
	     queries,
	     query_num,
	     found_knn
	     );
      printf("Query_bsf_snapshot_checked_nodes_count\t%lu\t%s\t%u\t%u\n",
	     bsf_snapshots[found_knn-1][j].checked_nodes,
	     queries,
	     query_num,
	     found_knn	       
	     );
      if (index->settings->classify)
	{
	  printf("Query_bsf_snapshot_label\t%u\t%s\t%u\t%u\n",
	     (unsigned int) bsf_snapshots[found_knn-1][j].label,
	     queries,
	     query_num,
	     found_knn	       
	     );
      ...
      if (index->settings->track_file_pos)
	{
	  printf("Query_bsf_snapshot_file_pos\t%u\t%s\t%u\t%u\n",
	     (unsigned int) bsf_snapshots[found_knn-1][j].file_pos,
	     queries,
	     query_num,
	     found_knn	       
	     );
	   ...
	   //printf("%u,%u,%u",query_num-1,found_knn-1,j);
           //printf("COUCOU");
             /*
               file_pos_raw = (((unsigned long) bsf_snapshots[found_knn-1][j].file_pos) - 1) * ts_length * sizeof(ts_type);
               fseek(dataset_file, 0L, SEEK_SET);
               fseek(dataset_file, file_pos_raw , SEEK_SET);
    	       fread(series, sizeof(ts_type), ts_length, dataset_file);
	       fprintf(series_file,"%u,%u,%u,%u",query_num,found_knn,j,(unsigned int) bsf_snapshots[found_knn-1][j].file_pos);
...
void print_query_stats(struct dstree_index * index, unsigned int query_num, unsigned int found_knn, char * queries)
{
      ...
      if (index->settings->classify)
	{
	  printf("Query_label\t%u\t%s\t%u\t%u\n",
		 (unsigned int) index->stats->query_label,
		 queries,
		 query_num,
		 found_knn	       
		 );
      ...
      if (index->settings->track_file_pos)
	{
	  printf("Query_file_pos\t%u\t%s\t%u\t%u\n",
		 (unsigned int) index->stats->query_file_pos,
		 queries,
		 query_num,
		 found_knn	       
		 );
