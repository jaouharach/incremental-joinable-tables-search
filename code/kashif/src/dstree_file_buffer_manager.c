...
enum response init_file_buffer_manager(struct dstree_index *index) {
  ...
  if (index->buffer_manager == NULL) {
    fprintf(stderr, "Error in dstree_file_buffer_manager.c: Could not allocate "
                    "memory for buffer manager.\n");
    return FAILURE;
  }

  index->buffer_manager->max_buffered_size = 1000 * 1000 * 100;

  index->buffer_manager->current_count =
      0; // the number of time series points in memory

  index->buffer_manager->batch_remove_size;

  index->buffer_manager->file_map = NULL; // the file map is empty initially
  index->buffer_manager->file_map_tail = NULL; // the file map is empty
                                               // initially
...
enum response set_buffered_memory_size(struct dstree_index *index) {
  if (index == NULL) {
    fprintf(
        stderr,
        "Error in dstree_file_buffer_manager.c: Cannot set the buffered memory \
                           for a NULL index.\n");
    return FAILURE;
  } else {
    ...
    index->buffer_manager->max_buffered_size =
        (long)(index->settings->buffered_memory_size * 1024 * 1024 /
               sizeof(ts_type));
    index->buffer_manager->batch_remove_size =
        index->buffer_manager->max_buffered_size / 2;
    ...
    */
    ...
    unsigned long num_leaf_buffers =
        2 * ((long)(num_bytes / (leaf_size * ts_size)));

    unsigned long size_leaf_buffer =
        sizeof(struct dstree_file_buffer) + (sizeof(ts_type *) * max_leaf_size);

    long long mem_array_size =
        (long long)((num_bytes - size_leaf_buffer * num_leaf_buffers) /
                    ts_size);

    index->buffer_manager->mem_array = calloc(mem_array_size, ts_size);
    if (index->buffer_manager->mem_array == NULL) {
      fprintf(stderr,
              "Error in dstree_file_buffer_manager.c:"
...
enum response get_file_buffer(struct dstree_index *index,
  ...
  if (node->file_buffer == NULL) // node does not have a file buffer yet
  {
    if (!dstree_file_buffer_init(node)) {
      fprintf(
          stderr,
          "Error in dstree_file_buffer_manager.c: Could not initialize the file buffer \
                           for the node.\n");
      return FAILURE;
    }
    ...
    if (!add_file_buffer_to_map(index, node)) {
      fprintf(
          stderr,
          "Error in dstree_file_buffer_manager.c: Could not add the file buffer \
                           to the map.\n");
      return FAILURE;
    }
  }

  //  to guard against the case that get_all_time_series is called while the
  //  buffer is almost full we always flush the buffer before it is completely
  //  full since get_all_time_series loads the full contents of a leaf size at
  //  once.

  int buffer_limit = index->buffer_manager->max_record_index -
                     (2 * index->settings->max_leaf_size);
  ...
  if (index->buffer_manager->current_record_index > buffer_limit) {
    ...
    while (currP != NULL) {
      ...
      if (!flush_buffer_to_disk(
              index,
              currP->file_buffer->node)) // flush the actual buffer of the node
      {
        fprintf(
            stderr,
            "Error in dstree_file_buffer_manager.c: Could not flush the buffer \
                          for node %s to disk.\n",
            currP->file_buffer->node->filename);
        return FAILURE;
      }
    ...
    memset(index->buffer_manager->mem_array, 0,
           index->buffer_manager->max_record_index);
    index->buffer_manager->current_record_index = 0;
    index->buffer_manager->current_record = index->buffer_manager->mem_array;
  }
  return SUCCESS;
...
enum response add_file_buffer_to_map(struct dstree_index *index,
                                     struct dstree_node *node) {
  ...
  if (idx == 0) {
    ...
    if (index->buffer_manager->file_map == NULL) {
      fprintf(stderr, "Error in dstree_file_buffer_manager.c: Could not "
                      "allocate memory for the file map.\n");
      return FAILURE;
    }
    ...
    */
    ...
    if (lastP->next == NULL) {
      fprintf(stderr, "Error in dstree_file_buffer_manager: Could not allocate "
                      "memory for new entry in the file map.\n");
      return FAILURE;
    }
...
enum response save_all_buffers_to_disk(struct dstree_index *index) {
  ...
  while (currP != NULL) {
    if (!flush_buffer_to_disk(index, currP->file_buffer->node)) {
      fprintf(
          stderr,
          "Error in dstree_file_buffer_manager.c: Saving all to disk..Could not flush the buffer \
                          for node %s to disk.\n",
          currP->file_buffer->node->filename);
      return FAILURE;
    }
