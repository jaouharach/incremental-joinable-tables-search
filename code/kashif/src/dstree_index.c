...
//enum response dstree_index_classify_insert(struct dstree_index *index,  ts_type * timeseries, unsigned char gt)
enum response dstree_index_classify_insert(struct dstree_index *index,  ts_type * timeseries, label_type gt, unsigned int fp)
{
	...
	qsort(child_node_points,num_child_node_points,sizeof(short),compare_short);
     ...
     if (!get_file_buffer(index, node))
      ...
      for (int idx=0; idx <index->settings->max_leaf_size;++idx)
      {
	   ...
	   //if(!append_ts_gt_to_child_node(index, node->left_child,ts_list[idx],node->gt[idx], node->file_pos[idx]))
	   if(!append_ts_gt_to_child_node(index, node->left_child,ts_list[idx], node, idx,fp))
	     {
	       fprintf(stderr,"Error in dstree_index.c: could not append \
                           time series to left child of			\
                           node %s\n", node->filename);
	   ...
	   //if(!append_ts_gt_to_child_node(index, node->right_child,ts_list[idx],node->gt[idx],node->file_pos[idx]))
	   if(!append_ts_gt_to_child_node(index, node->right_child,ts_list[idx],node, idx, fp))
	     {
	       fprintf(stderr,"Error in dstree_index.c: could not append \
                           time series to right child of		\
                           node %s\n", node->filename);
...
enum response dstree_index_write(struct dstree_index *index)
{
	...
	if (index->settings->classify)
	{
	  const char *gt_filename = malloc(sizeof(char) * (strlen(index->settings->root_directory) + 8));
	...
	if (index->settings->track_file_pos)
	  {
	    const char *fp_filename = malloc(sizeof(char) * (strlen(index->settings->root_directory) + 8));
...
struct dstree_index * dstree_index_read(const char* root_directory)
{
	...
	index->settings->track_file_pos = track_file_pos;	
        ...
        if(index->settings->classify)
	{
	  const char *gt_filename = malloc(sizeof(char) * (strlen(index->settings->root_directory) +7));
	  strcpy(gt_filename, index->settings->root_directory);
	  strcat(gt_filename, "gt.idx\0");
	  ...
	  //fread(index->gt_cache, sizeof(unsigned char), dataset_size, index->gt_file);	  
	  ...
	  fread(index->gt_cache, sizeof(label_type), dataset_size, index->gt_file);	  
        ...
        if(index->settings->track_file_pos)
	{
	  const char *fp_filename = malloc(sizeof(char) * (strlen(index->settings->root_directory) +7 ));
	  strcpy(fp_filename, index->settings->root_directory);
	  strcat(fp_filename, "fp.idx\0");
		...
		if(node->filename != NULL) {
			...
			flush_buffer_to_disk(index,node);
			 if (index->settings->classify)
			 {
			   ...
			   //fwrite(node->gt, sizeof(unsigned char), node->node_size, index->gt_file);
...
void cache_gt_file(struct dstree_index *index) {
    COUNT_INPUT_TIME_START
      //fread(index->gt_cache, sizeof(unsigned char), index->first_node->node_size, index->gt_file);
	fread(index->gt_cache, sizeof(label_type), index->first_node->node_size, index->gt_file);
