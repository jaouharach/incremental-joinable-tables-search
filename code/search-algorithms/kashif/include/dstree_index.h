...
struct stats_info {
  ...
  */
  ...
  label_type query_label;
  unsigned int query_file_pos;
  ...
  unsigned long long
      tlb_ts_count; // this could be different from the total_ts_count. It
                    // includes only the ts for whiche d(Q,S) != 0
  unsigned long long
      eff_epsilon_queries_count; // the number of queries for which  d(Q,S) != 0

  unsigned int total_queries_count; // this could be different from the input
                                    // queries to eliminate cases d(Q,S) = 0
...
struct dstree_index {
  ...
  FILE *gt_file;
  unsigned int gt_pos_ctr;
  // unsigned char * gt_cache;
  label_type *gt_cache;
  unsigned int gt_cache_size;

  const char *fp_filename;
  FILE *fp_file;
  unsigned int fp_pos_ctr;
  // unsigned char * gt_cache;
...
struct dstree_index_settings *dstree_index_settings_init(
    ...
    double buffered_memory_size, boolean is_index_new, const char *);
...
enum response dstree_index_insert(struct dstree_index *index,
                                  ts_type *timeseries);
enum response append_ts_to_buffered_list(struct dstree_index *index,
...
// struct dstree_ts_buffer * copy_ts_in_mem(struct dstree_index * index, struct
// dstree_node * node);
