...
namespace minmax_heap_helpers
{
    ...
    uint64_t smallest_descendant(It begin, uint64_t length, uint64_t first_child, uint64_t first_grandchild, Compare && compare)
    {
        ...
        if (first_grandchild >= length)
            return first_child + (second_child != length && compare(begin[second_child]->distance, begin[first_child]->distance));
        uint64_t second_grandchild = first_grandchild + 1;
        if (second_grandchild == length)
            return compare(begin[first_grandchild]->distance, begin[second_child]->distance) ? first_grandchild : second_child;
        uint64_t min_grandchild = first_grandchild + !!compare(begin[second_grandchild]->distance, begin[first_grandchild]->distance);
        uint64_t third_grandchild = second_grandchild + 1;
        if (third_grandchild == length)
            return compare(begin[min_grandchild]->distance, begin[second_child]->distance) ? min_grandchild : second_child;
        else
            return compare(begin[min_grandchild]->distance, begin[third_grandchild]->distance) ? min_grandchild : third_grandchild;
    }
    ...
    uint64_t largest_descendant(It begin, uint64_t length, uint64_t first_child, uint64_t first_grandchild, Compare && compare)
    {
        uint64_t second_child = first_child + 1;
        if (first_grandchild >= length)
            return first_child + (second_child != length && compare(begin[first_child]->distance, begin[second_child]->distance));
        uint64_t second_grandchild = first_grandchild + 1;
        if (second_grandchild == length)
            return compare(begin[second_child]->distance, begin[first_grandchild]->distance) ? first_grandchild : second_child;
        uint64_t max_grandchild = first_grandchild + !!compare(begin[first_grandchild]->distance, begin[second_grandchild]->distance);
        uint64_t third_grandchild = second_grandchild + 1;
        if (third_grandchild == length)
            return compare(begin[second_child]->distance, begin[max_grandchild]->distance) ? max_grandchild : second_child;
        else
            return compare(begin[max_grandchild]->distance, begin[third_grandchild]->distance) ? third_grandchild : max_grandchild;
    }
    ...
    void push_down_min(It begin, typename std::iterator_traits<It>::value_type value, uint64_t index, uint64_t length, Compare && compare)
    {
        ...
        for (;;)
        {
            ...
            if (last_grandchild < length)
            {
                ...
                uint64_t min_first_half = last_grandchild - 2 - !!compare(it[-3]->distance, it[-2]->distance);
                uint64_t min_second_half = last_grandchild - !!compare(it[-1]->distance, it[0]->distance);
                uint64_t smallest = compare(begin[min_second_half]->distance, begin[min_first_half]->distance) ? min_second_half : min_first_half;
                if (!compare(begin[smallest]->distance, value->distance))
                    break;
                begin[index] = std::move(begin[smallest]);
                index = smallest;
                uint64_t parent = parent_index(index);
                if (compare(begin[parent]->distance, value->distance))
    ...
    void push_down_min_one_child_only(It begin, uint64_t index, Compare&& compare)
    {
        ...
        if (compare(begin[child]->distance, begin[index]->distance))
    ...
    void push_down_min_one_level_only(It begin, uint64_t index, Compare&& compare)
    {
        using std::swap;
        uint64_t first_child = first_child_index(index);
        uint64_t smaller_child = first_child + !!compare(begin[first_child + 1]->distance, begin[first_child]->distance);
        if (compare(begin[smaller_child]->distance, begin[index]->distance))
    ...
    void push_down_max(It begin, typename std::iterator_traits<It>::value_type value, uint64_t index, uint64_t length, Compare&& compare)
    {
        ...
        for (;;)
        {
            ...
            {
                auto it = begin + last_grandchild;
                uint64_t max_first_half = last_grandchild - 2 - !!compare(it[-2]->distance, it[-3]->distance);
                uint64_t max_second_half = last_grandchild - !!compare(it[0]->distance, it[-1]->distance);
                uint64_t largest = compare(begin[max_first_half]->distance, begin[max_second_half]->distance) ? max_second_half : max_first_half;
                if (!compare(value->distance, begin[largest]->distance))
                    break;
                begin[index] = std::move(begin[largest]);
                index = largest;
                uint64_t parent = parent_index(index);
                if (compare(value->distance, begin[parent]->distance))
                    swap(begin[parent], value);
            }
            else
            {
    ...
    void push_down_max_one_child_only(It begin, uint64_t index, Compare&& compare)
    {
        using std::swap;
        uint64_t child = first_child_index(index);
        if (compare(begin[index]->distance, begin[child]->distance))
            swap(begin[index], begin[child]);
    }
    ...
    void push_down_max_one_level_only(It begin, uint64_t index, Compare&& compare)
{
    using std::swap;
    uint64_t first_child = first_child_index(index);
    uint64_t bigger_child = first_child + !!compare(begin[first_child]->distance, begin[first_child + 1]->distance);
    if (compare(begin[index]->distance, begin[bigger_child]->distance))
...
bool is_minmax_heap(It begin, It end, Compare&& compare)
{
    ...
    for (uint64_t i = 0; i < length; ++i)
    {
        if (minmax_heap_helpers::is_min_item(i))
        {
            ...
            {
                return child >= length || !compare(begin[child]->distance, begin[i]->distance);
            };
        ...
        else
        {
            auto compare_one = [&](uint64_t child)
            {
                return child >= length || !compare(begin[i]->distance, begin[child]->distance);
            };
            if (!test_index(i, compare_one))
                return false;
        }
...
void push_minmax_heap(It begin, It end, Compare&& compare)
{
    ...
    if (minmax_heap_helpers::is_new_item_min(length))
    {
        if (index == 0)
            static_cast<void>(0);
        else if (compare(begin[parent]->distance, value->distance))
        {
            begin[index] = std::move(begin[parent]);
            index = parent;
            goto push_up_max;
            ...
            {
                {
                    ...
                    if (compare(value->distance, begin[grandparent]->distance))
                    {
...
push_up_min:
    ...
    else if (compare(value->distance, begin[parent]->distance))
    {
        begin[index] = std::move(begin[parent]);
        index = parent;
        goto push_up_min;
...
push_up_max:
        ...
        {
            uint64_t grandparent = minmax_heap_helpers::grandparent_index(index);
            if (compare(begin[grandparent]->distance, value->distance))
            {
                begin[index] = std::move(begin[grandparent]);
                index = grandparent;
            }
...
void pop_minmax_heap_max(It begin, It end, Compare&& compare)
{
    ...
    uint64_t index = 1 + !!compare(begin[1]->distance, begin[2]->distance);
