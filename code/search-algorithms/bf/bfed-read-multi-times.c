...
{   
    ...
    printf("Datalake size:\t\t\t%uGB\n\nTotal table:\t\t\t%u\n\nNumber of queries:\t\t%u\n\nMin query size:\t\t\t%u\n\nMax query size:\t\t\t%u", dlsize, l, qset_num, min_qset_size, max_qset_size);
    return 0;
}
    ...
    )
{
    ...
    struct query_result * all_knn_results = malloc(0);
    struct query_result * curr_knn = calloc(k, sizeof(struct query_result));;
        ...
        if(is_binaryfile(dfile->d_name))
        {
            ...
            while(total_bytes)
            {
                ...
                { 
                    ...
                    total_checked_vec = 0;
                    qset_num--;
                    all_knn_results = realloc(all_knn_results, k * nvec * sizeof(struct query_result));
                    knn_array_idx = 0;                    
                ...
                else if(i <= (unsigned int)nvec*vector_length)
                {
                    ...
                    if(j > 99){
                        ...
                        brute_force_knn_search(bin_files_directory, l, vector_length,query_vector, k, &query_time, &total_checked_vec, curr_knn);

                        //append new knn(s) to knn_results array
                        for(int i = 0; i<k; i++)
                        {
                            all_knn_results[knn_array_idx++] = curr_knn[i];
                        }
                    }
                    ...
                    {   
                        ...
                        //append new knn(s) to knn_results array
                        for(int p = 0; p<k; ++p){
                            all_knn_results[knn_array_idx++] = curr_knn[p];
                        }
...
struct query_result * brute_force_knn_search(char * bin_files_dir, unsigned int l, unsigned int vector_length, struct Vector q, unsigned int k, double * query_time, unsigned int *total_checked_vec, struct query_result *knn_results)
{
    ts_type bsf = FLT_MAX;
    unsigned int found_knn = 0;
    unsigned int next_knn = 0;

    //queue containing kNN results
    //struct query_result *knn_results = calloc(k, sizeof(struct query_result));
    for (int i = 0; i < k; ++i)
    {
        knn_results[i].vector_id = malloc(sizeof(struct vid));
        knn_results[i].vector_id->set_id = 0;
        knn_results[i].vector_id->table_id = 0;
        knn_results[i].distance = FLT_MAX;
    }
    ...
    while ((dfile = readdir(dir)) != NULL && l > 0)
    {
        ...
        if(is_binaryfile(dfile->d_name))
        {
            ...
            //printf("Reading file: %s, Total bytes = %u\n\n", dfile->d_name, total_bytes);
            ...
            while(total_bytes)
            {
                ...
                { 
                    ...
                    if(v.table_id == q.table_id && v.set_id == q.set_id)
                    {
                ...
                else if(i <= (unsigned int)nvec*vector_length)
                {
                    ...
                    {    
                        ...
                        j = 0;
                        d = euclidian_distance(q.values, v.values, vector_length);
                        *total_checked_vec += 1;
                        ...
                        if (d < bsf)
                        {
                            ...
                            for(int m = 1; m < k; ++m)
                            {
                                knn_results[m].distance = knn_results[m-1].distance;
                                knn_results[m].vector_id->set_id = knn_results[m-1].vector_id->set_id;
                                knn_results[m].vector_id->table_id = knn_results[m-1].vector_id->table_id;
                            }

                            knn_results[0].distance = bsf;
                            knn_results[0].vector_id->set_id = v.set_id;
                            knn_results[0].vector_id->table_id = v.table_id;
                            next_knn = 1;

                            
                        }
                        // if its another vector with same bsf
                        else if(d == bsf && next_knn < k)
                        {
                            knn_results[next_knn].distance = bsf;
                            knn_results[next_knn].vector_id->set_id = v.set_id;
                            knn_results[next_knn].vector_id->table_id = v.table_id;
                            ++next_knn;
                        }
                    ...
                    {   
                        ...
                        // process last vector in current set
                        d = euclidian_distance(q.values, v.values, vector_length);
                        *total_checked_vec = *total_checked_vec + 1;
                        // if current vector is closer to query vector
                        if (d < bsf)
                        {
                            bsf = d;
